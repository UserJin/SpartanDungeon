...
public class NPC : MonoBehaviour, IDamagable
{
    ...
    void WanderToNewLocation()
    {
        ...
        SetState(AIState.Wandering);
        agent.SetDestination(GetWanderLoaction());
    }

    Vector3 GetWanderLoaction()
    {
        NavMeshHit hit;
    ...
    void Die()
    {
        for(int i = 0; i < dropOnDetah.Length;i++)
        {
            Instantiate(dropOnDetah[i].dropPrefab, transform.position + Vector3.up * 2, Quaternion.identity);
        }
        Destroy(gameObject);
    }
